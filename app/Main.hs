...
writeIR h allocRegisters (fName, g) = do
    ...
    hPutStrLn h "graph:\n"
    let showG i = let name = bbLabel ((ids g) M.! i)
                      names = map (bbLabel . ((ids g) M.!)) ((next g) M.! i)
                    in hPutStrLn h (show name ++ " -> " ++ show names)
    mapM_ showG (M.keys (ids g))
    hPutStrLn h ("start: " ++ show (bbLabel ((ids g) M.! (start g))))
    hPutStrLn h ("end: " ++ show (map (\i -> bbLabel ((ids g) M.! i)) (end g)))
    hPutStrLn h "\n\n"

writeCodeMap :: Handle -> M.Map VIdent [AssemblerInstr] -> IO ()
...
writeCode :: Handle -> (VIdent, [AssemblerInstr]) -> IO ()
...
compileProgram :: OptimizationOptions -> String -> Err (Program (), M.Map VIdent BBGraph,  M.Map VIdent [AssemblerInstr], ProgramIR)
    ...
    case compileProgram options fileContent of
      ...
      Ok (program, ir, code, programir) -> do
          ...
          when (saveIR args) (do
              ...
              hPutStrLn stderr ("Writing IR to " ++ fileIR)
          ...
          hClose handleCode
          callCommand (getNasmCommand args fileObj fileCode)
          callCommand (getGccCommand args fileOut fileObj)
